...
public class VSTS_10339_QA extends BaseClass_EUser {
	...
	public void testmodals(boolean bugflag, boolean exceptionflag) throws Exception {
		try {
			...
			ObjectHelper.driver.switchTo().frame("frame1");
			Thread.sleep(3000);
			executor.executeScript("arguments[0].click();", ObjectHelper.driver.findElement(By.id("selbox")));
			ObjectHelper.driver.switchTo().defaultContent();
			ObjectHelper.driver.switchTo().frame("ifRecords");
			Thread.sleep(3000);
			ObjectHelper.driver.findElement(By.id("tdPathologyResetStatusButton")).click();
			Thread.sleep(3000);
